...
def main():
    filename="/{}/out/analytic_survival_Copenhagen_filtered_15min.h5".format(base_path)
    ...
    survival = np.array(g["data/infectious_3.00_latent_2.00/survival_probability_p/"])
    ax.plot(survival[0],survival[2], label=r"data with $T_\mathrm{{lat}}=2$", color=clrs.n_low)
    survival = np.array(g["data/infectious_3.00_latent_6.00/survival_probability_p/"])
    ax.plot(survival[0],survival[2], label=r"data with $T_\mathrm{{lat}}=6$", color=clrs.n_high)
    survival = np.array(g["rand/infectious_3.00_latent_2.00/survival_probability_p/"])
    ax.plot(survival[0],survival[2], label=r"data with $T_\mathrm{{lat}}=2$", color=clrs.n_psn)
    survival = np.array(g["rand/infectious_3.00_latent_6.00/survival_probability_p/"])
    ax.plot(survival[0],survival[2], '-.', label=r"data with $T_\mathrm{{lat}}=6$", color=clrs.n_psn)
